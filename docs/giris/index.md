# Papirus-AI Dokümantasyonu

Papirus-AI, akademisyenlerin yazılı ve açık uçlu sınav kağıtlarını yapay zekâ desteğiyle değerlendirmesine yardımcı olan modern bir sınav değerlendirme platformudur.

Klasik yazılı sınav süreçleri; kağıt okuma, puanlama, kontrol etme ve sonuçları düzenleme gibi zaman alan işlemler içerir. Papirus-AI bu süreci dijitalleştirerek değerlendirme süreçlerini daha hızlı, daha düzenli ve daha ölçeklenebilir hale getirir.

---

## Papirus-AI ile Neler Yapabilirsiniz?

- Yazılı sınav kağıtlarını sisteme yükleyebilirsiniz
- Açık uçlu sorular için değerlendirme kriterleri oluşturabilirsiniz
- AI destekli ön değerlendirme başlatabilirsiniz
- Sonuçları manuel olarak kontrol edip düzenleyebilirsiniz
- Öğrenci bazlı ve soru bazlı analizler görüntüleyebilirsiniz
- Sonuçları dışa aktarabilirsiniz

---

## Platform Nasıl Çalışır?

Papirus-AI temel olarak aşağıdaki iş akışını takip eder:

```txt
1. Sınav oluştur
2. Soruları tanımla
3. Rubric oluştur
4. Öğrenci kağıtlarını yükle
5. AI değerlendirmesini başlat
6. Sonuçları incele
7. Rapor oluştur
```

---

## Desteklenen Özellikler

### Yapay Zekâ Destekli Değerlendirme
Sistem, öğrenci cevaplarını tanımlanan rubric ve cevap anahtarlarına göre analiz eder.

### Açık Uçlu Soru Analizi
Klasik test sistemlerinden farklı olarak açık uçlu cevapların değerlendirilmesini destekler.

### El Yazısı ve OCR Desteği
Taranmış sınav kağıtları ve görseller üzerinde analiz yapılabilir.

### Manuel Kontrol Mekanizması
Akademisyenler AI tarafından verilen puanları inceleyebilir ve düzenleyebilir.

---

## Akademik Kullanım İçin Tasarlandı

Papirus-AI özellikle:
- Üniversiteler
- Özel okullar
- Kurs merkezleri
- Açık uçlu sınav değerlendirmesi yapan eğitim kurumları

için geliştirilmiştir.

---

## Sonraki Adımlar

Başlangıç için aşağıdaki dokümanları inceleyebilirsiniz:

- Hızlı Başlangıç
- İlk Sınav Oluşturma
- Rubric Sistemi
- AI Değerlendirme Süreci
- Sonuç İnceleme ve Raporlama